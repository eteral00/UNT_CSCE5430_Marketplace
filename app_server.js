...
app_server.get("/admin$", (req, res) => {
  try {
    console.log("GET-Admin hit. Redirected to get-admin-page");
  ...
  try {
    ...
    findUserByUsername(username, function(resultList) {
      if (resultList.length < 1) {
        res.status(400).json({ message : "Sorry. Either the system could not find any user with username matching '" + username + "', or it ran into some errors!" });
      } else {
        ...
        for (const record of resultList) {
          ...
          user.emailAddress = record.email_address;
          user.phoneNumber = record.phone_number;
          user.firstName = record.first_name;
          user.lastName = record.last_name;
          user.isLocked = Boolean(record.is_locked.toJSON().data[0]);
          user.lockedUntilDate = record.locked_until_date;
...
app_server.put("/admin/manage_user/modify", (req, res) => {
  try {
    console.log("PUT-admin_modify_user hit");
    
    let inputUsername = req.body.username;
    let inputFName = req.body.firstName;
    let inputLName = req.body.lastName;
    let inputEmail = req.body.emailAddress;
    let inputPhone = req.body.phoneNumber;
    ...
    if (prior_term)
    {
      ...
      queryVar.push(inputUsername);
      // send query
      mySQL_DbConnection.query(queryStr, queryVar, function (err, result_rows, fields) {             
        if(err) {
          console.log("Error: ", err);
          res.status(409).json({ message : "Sorry! There were errors updating the profile of user '" + inputUsername + "'!" });

        } else {
          console.log("Successfully updated the profile of user '" + sessionOb.user.username + "'!");
          res.status(200).json({ message : "Successfully updated the profile of user '" + inputUsername + "'!" });

        }
      });
    } else {
      res.status(400).json({ message : "Sorry! There were errors updating the profile of user '" + inputUsername + "'!" });
    }
...
// PUT-admin_lock_user
app_server.put("/admin/manage_user/lock", (req, res) => {
  try {
    console.log("PUT-admin_lock_user hit");
    
    sessionOb = req.session;
    
    let username = req.body.username;
    viewUserByUsername (username, function (result_rows) {
      if (result_rows.length < 1) {
        // user not found
        res.status(400).json({ message : "The requested user, '" + username + "', does not exist!" });
      } else {
        // user found => blocking
        toggleLockUser(username, true, function(updateResult){
          if (!updateResult) {
...
// PUT-admin_unlock_user
app_server.put("/admin/manage_user/unlock", (req, res) => {
  try {
    console.log("PUT-admin_unlock_user hit");
  ...
  try {
    ...
    mySQL_DbConnection.query(queryStr, queryVar, function (err, result_rows, fields) {             
      if(err) {
          console.log("Error: ", err);
          //res.status();
      } else if (result_rows.length < 1) {
        // no record found, user may not exist
        res.status(400).json({ message : "The requested user, '" + userName + "', does not exist!" });
      } else {
        // user found, changing password
